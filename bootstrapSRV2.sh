...
echo "SRV2" > /etc/hostname
sudo echo "127.0.0.1 SRV2"> /etc/hosts
...
sudo debconf-set-selections <<< 'mysql-server-5.5 mysql-server/root_password password Q'
sudo debconf-set-selections <<< 'mysql-server-5.5 mysql-server/root_password_again password Q'
...
sudo mysql -u root --password=Q -e "CHANGE MASTER TO MASTER_HOST='192.168.10.11',MASTER_USER='sync', MASTER_PASSWORD='Q', MASTER_LOG_FILE='mysql-bin.000001', MASTER_LOG_POS=  107;"
